...
export class Context {
     ...
     * Accessing a key that was not connected returns `null`.
     *
     * The concrete type of each key comes from the consumer project's module augmentation
     * of `Context` in `src/types/Context.d.ts` (generated by `morphis new` / `morphis new:connection`).
     *
     * @example
     * // router.use([Connect('default'), Connect('mysql')])
     * const posts = await current.db['default'].select().from(Post.table);
     * current.db['mongo']; // null — Connect('mongo') was not used
     */
    // `db` is intentionally not declared here as a class field so that the consumer
    // project's module augmentation (interface Context { db: ... }) is the sole type
    // source, preventing `any` from shadowing it. The proxy is initialised at runtime.
    constructor() {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (this as any).db = new Proxy({} as Record<string, any>, {
            get(target: Record<string, any>, prop: string | symbol) {
                if (typeof prop === 'symbol') return undefined;
                if (prop === 'then') return undefined;
                return Object.prototype.hasOwnProperty.call(target, prop) ? target[prop as string] : null;
            },
        });
    }
}
